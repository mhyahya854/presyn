...
from backend.app.db.enums import VisitorStatus
...
class Visitor(Base, TimestampMixin):
    ...
    status: Mapped[VisitorStatus] = mapped_column(
        SQLEnum(VisitorStatus, native_enum=False, create_constraint=True, validate_strings=True, length=30),
        default=VisitorStatus.EXPECTED,
        nullable=False,
        index=True,
    )
    events: Mapped[List[VisitorEvent]] = relationship(
        "VisitorEvent", back_populates="visitor", cascade="all, delete-orphan"
    )
