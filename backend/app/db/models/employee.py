...
from backend.app.db.enums import EmployeeStatus, EnrollmentStatus
...
class Employee(Base, TimestampMixin):
    ...
    status: Mapped[EmployeeStatus] = mapped_column(
        SQLEnum(EmployeeStatus, native_enum=False, create_constraint=True, validate_strings=True, length=20),
        default=EmployeeStatus.ACTIVE,
        nullable=False,
        index=True,
    )
    enrollment_status: Mapped[EnrollmentStatus] = mapped_column(
        SQLEnum(EnrollmentStatus, native_enum=False, create_constraint=True, validate_strings=True, length=30),
    ...
    face_templates: Mapped[List[FaceTemplate]] = relationship(
        "FaceTemplate", back_populates="employee", cascade="all, delete-orphan"
    )
